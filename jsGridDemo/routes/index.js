...
//prepare the "cities" record collection
var cities = [];
db.regioni.forEach(function (reg) {
    for (var i = 0; i < reg.capoluoghi.length; i++) {
        cities.push({
            name: reg.capoluoghi[i],
            short: reg.province[i],
            region: reg.nome
        });
    }
});
...
router.get('/cities', function (req, res) {
    ...
    if (req.query.sortField) {
        sorted = _.orderBy(cities, [req.query.sortField], [req.query.sortOrder]);
    }
    else {
        sorted = cities;
    }
    ...
    var offset = (+req.query.pageIndex - 1) * size;
    var recs = size ? sorted.slice(offset, offset + size) : sorted;
    var result = {
        data: recs,
